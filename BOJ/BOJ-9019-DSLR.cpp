// (-1 % 10000)은 9999가 아니고 -1로 나온다는 사실 명심..
...
void find(){
    ...
    while(!bfs.empty()){
        ...
        //D
        if((set[(idx * 2) % 10000].index == M || set[(idx * 2) % 10000].index > set[idx].index + 1) && idx){
            tempS = set[idx].str;
            tempS += "D";
        ...
        if((set[tempI].index == M || set[tempI].index > set[idx].index + 1) && idx){
            tempS = set[idx].str;
            tempS += "L";
            set[tempI].index = set[idx].index + 1;
            set[tempI].str = tempS;
